...
import { currentLocalHour, todayKey } from "./time";
  ...
  try {
    ...
    for (const participant of listPushParticipants()) {
      ...
      if (hour >= FOLLOWUP_HOUR && participant.last_followup_day !== today) {
        const reconstruction = getReconstruction(participant.username, today);
        if (reconstruction?.status === "submitted") continue;
        setLastFollowupDay(participant.username, today);
        messages.push({
          to: participant.push_token,
...
export function startPushScheduler(): void {
  ...
  console.log(
    `Push scheduler started (reminder ${REMINDER_HOUR}:00, follow-up ${FOLLOWUP_HOUR}:00 local study time)`,
  );
}
