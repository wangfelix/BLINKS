...
app.get("/api/profile", auth_1.requireAuth, (req, res) => {
    ...
    const plan = (0, db_1.parseConditionPlan)(participant?.condition_plan);
    res.json({
...
app.put("/api/profile", auth_1.requireAuth, (req, res) => {
    const { occupation, workDescription } = req.body;
    ...
    (0, db_1.upsertParticipantProfile)(req.participant, occupation.trim(), workDescription.trim());
    res.json({ ok: true });
});
...
const resolveDayCondition = (participant, day) => {
    const reconstruction = (0, db_1.getReconstruction)(participant, day);
    if (reconstruction)
        return reconstruction.condition;
    const dayIndex = (0, db_1.aggregateFrameDays)(participant).findIndex((aggregate) => aggregate.day === day);
    if (dayIndex === -1)
        return undefined;
    const plan = (0, db_1.parseConditionPlan)((0, db_1.getParticipant)(participant)?.condition_plan);
...
const parseActivityInputs = (body, day, requireLabels) => {
    ...
    for (const [index, item] of list.entries()) {
        ...
        const vlmRawLabel = typeof entry.vlmRawLabel === "string" && entry.vlmRawLabel.length > 0
            ? entry.vlmRawLabel
            : null;
        const vlmCategory = CATEGORY_LABELS.has(entry.vlmCategory)
            ? entry.vlmCategory
            : null;
        activities.push({
            start_ms: startMs,
...
app.get("/api/reconstruction/days", auth_1.requireAuth, (req, res) => {
    const participant = req.participant;
    const plan = (0, db_1.parseConditionPlan)((0, db_1.getParticipant)(participant)?.condition_plan);
    const days = (0, db_1.aggregateFrameDays)(participant)
        .map((aggregate, index) => {
        const reconstruction = (0, db_1.getReconstruction)(participant, aggregate.day);
        return {
            day: aggregate.day,
            dayNumber: index + 1,
            condition: reconstruction?.condition ?? plan[Math.min(index, plan.length - 1)],
...
app.get("/api/reconstruction/:day", auth_1.requireAuth, (req, res) => {
    const participant = req.participant;
    ...
    (0, db_1.pinReconstructionCondition)(participant, day, condition);
    let reconstruction = (0, db_1.getReconstruction)(participant, day);
    let activities = (0, db_1.listActivities)(participant, day);
    ...
    const vlmPendingCount = dayFrames.filter((f) => (f.vlm_status === "pending" || f.vlm_status === "processing") &&
    ...
    if (condition === "assisted" &&
        ...
        const segments = (0, segmentation_1.segmentDay)(servedFrames.map((frame) => ({
            captureEpochMs: frame.capture_epoch_ms,
            vlmLabel: frame.vlm_status === "done" ? frame.vlm_label : null,
            vlmCategory: frame.vlm_status === "done" &&
        ...
        reconstruction = (0, db_1.getReconstruction)(participant, day);
        activities = (0, db_1.listActivities)(participant, day);
        console.log(`Generated initial segmentation: ${participant}/${day} (${activities.length} activities)`);
    }
    const payload = {
        day,
        condition,
...
const guardReconstructionWrite = (req, res) => {
    const day = req.params.day;
    if (!DAY_KEY_PATTERN.test(day)) {
        res.status(400).json({ error: "day must be YYYY-MM-DD" });
        return undefined;
    ...
    if ((0, db_1.getReconstruction)(req.participant, day)?.status === "submitted") {
...
app.put("/api/reconstruction/:day", auth_1.requireAuth, (req, res) => {
...
app.post("/api/reconstruction/:day/submit", auth_1.requireAuth, (req, res) => {
    const guard = guardReconstructionWrite(req, res);
    if (!guard)
        return;
    ...
    console.log(`Reconstruction submitted: ${req.participant}/${guard.day} (${activities.length} activities)`);
