...
function migrateDropColumn(table, column) {
...
function initDb(dbPath) {
  ...
  `);
    recordRecordingEventTx = db.transaction((participant, event, serverReceivedEpochMs) => {
        const byId = getRecordingEventByIdStmt.get(participant, event.event_id);
        const bySequence = getRecordingEventBySequenceStmt.get(participant, event.session, event.sequence_number);
        const existing = byId ?? bySequence;
        if (existing) {
            const identical = existing.event_id === event.event_id &&
            ...
            if (!identical) {
                throw new RecordingEventConflictError("recording event identity conflicts with an existing event");
            }
            return existing;
        }
        insertRecordingEventStmt.run(participant, event.event_id, event.session, event.event_type, event.client_epoch_ms, serverReceivedEpochMs, event.sequence_number);
        return getRecordingEventByIdStmt.get(participant, event.event_id);
    });
    // --- DRM statements --------------------------------------------------------
...
function recordRecordingEvent(participant, event) {
...
function closeFillingChunksForSession(participant, session) {
