...
- **Web app (`drm-web/`):** Next.js + shadcn on **blinks.win.kit.edu** next to the API. Linear pipeline: (1) first login onboarding — mandatory password replacement, then the external pre-study LimeSurvey link with `participant_id`; completion stays on its success screen until the participant logs out; (2) a welcome/instruction screen before **/reconstruct** mounts the self editor (manual time spans + activity/category dropdowns, **no frames, no VLM output**); (3) after self submission, a second instruction screen appears before the assisted editor mounts (rows with frames, time span, the same activity/category dropdowns; delete row, insert row between, re-pick start/end frames with neighbor spans adjusting); (4) external final LimeSurvey link with `participantId`; (5) offboarding page. The DRM progress indicator remains visible on both instruction screens. Surveys are never embedded in iframes.
...
**DRM web developer navigator (2026-07-19, onboarding preview added 2026-08-02):** start both the API server and `drm-web` with `DRM_DEV_MODE=1` (or run `DRM_DEV_MODE=1 ./dev-all.sh`) to show a floating menu linking directly to a non-mutating onboarding preview, the self editor, assisted editor, survey page, and offboarding. The preview is fully interactive but never changes the signed-in account. Dev mode bypasses only the evening gate and the round-1-before-round-2 gate; authentication, participant isolation, autosave/submission, and submitted-round finality remain enforced. Never enable it during the study.
...
- **Auth:** separate **`server/data/auth.db`** (NOT in `recordings/` — backups/rsyncs of research data must never carry credentials). `users` (username PK = participant id, argon2id `password_hash`, `created_at`, `must_change_password`, nullable `onboarding_completed_at`) + `auth_tokens` (sha256-hashed opaque 32-byte tokens, no expiry for the short study, revocation = row delete). New accounts require the first-run web flow; migrated existing accounts are grandfathered as completed. The token-authenticated onboarding password endpoint does not ask for the just-used lab password, while the general password endpoint still does. Login burns comparable argon2 time for unknown users (no username oracle). Provisioning: `npm run create-user -- <username> <password> [--reset]`; full wizard reset: `npm run reset-onboarding -- <username>` — no self-signup.
...
npm run reset-onboarding -- participant1      # repeat both first-run steps
