...
- **Mobile app:** the recorder, now single-day (no multi-day progress UI); onboarding collects **occupation + work description + wake/bed times** (stored server-side; occupation is the VLM's classification context, bedtime drives the fallback push). The **Photos** tab keeps the session list visible, but selecting a session shows a restriction notice until Self DRM is submitted; afterward it becomes a virtualized three-column gallery with authenticated preview and deletion. The app never receives VLM output.
- **VLM worker (`server/vlm/`):** per **5-minute chunk** elicits two complete verbalized probability distributions: all 17 closed, visually grounded activity enums and the independent categories `work | break | other`. The worker derives both stored labels by deterministic argmax and stores both maximum probabilities and both full normalized dictionaries. `break` = intentional, restorative pause ("erholsame Pause"); `other` = neither work nor restorative. The prompt adapts Wang et al., _Calibrating Verbalized Probabilities for Large Language Models_ (arXiv:2410.06707v1): determine the label internally, assess confidence, return the full distribution, and constrain the output. It uses strict JSON Schema rather than the paper's Python-dict example, returns only the two distributions, and defaults to temperature 0. Local validation requires exact key sets, values in `[0,1]`, and an approximate sum of 1 before exact normalization. These black-box self-assessments are not calibrated probabilities; the paper supports the elicitation method, not this VLM's calibration, the intervention's validity, or the `0.8` threshold. Category classification is conditioned on the participant's occupation/work description. No scene setting, description, or descriptor/vector is inferred or stored.
...
- **Web app (`drm-web/`):** Next.js + shadcn on **blinks.win.kit.edu** next to the API. Linear pipeline: (1) first-login onboarding — mandatory password replacement, then the external pre-study LimeSurvey link with `participant_id`; the success screen remains until the participant explicitly signs out; (2) after the recording day, a welcome/instruction screen appears before **/reconstruct** mounts the self editor (manual time spans + activity/category dropdowns, **no frames, no VLM output**); the self editor's time fields take a bare wall-clock `HH:MM` and **never show a date**, but a typed time that would fall before what the participant already entered is stored on the next calendar date (`resolveTypedTimeOfDay` + `selfTimeAnchor` in `drm-web`), so a day ending at 00:30 reconstructs correctly from chronology alone. The roll is taken only when the result still fits the day's extent, which confines it to the small hours: an out-of-order daytime entry keeps its own date and surfaces as the ordinary overlap warning. Covered by `drm-web/scripts/test-self-time-entry.ts`; (3) after self submission, an assisted-round instruction screen explains the proposed reconstruction and warns that some Step-2 labels/types are deliberately incorrect before mounting the assisted editor (rows with frames, time span, the same activity/category controls, delete/insert/merge, and start/end frame adjustment); (4) the external final LimeSurvey opens with `participantId`, and the local completion CTA appears only after its link was opened; (5) confirming questionnaire completion persists the terminal study state and routes to **`/done`**. Every later login or protected-page visit returns to `/done`; reconstruction pages/APIs stay closed, while the navbar's **Manage Photos** dialog remains available for viewing and deleting study photos. The DRM progress indicator remains visible on both instruction screens. Surveys are never embedded in iframes.
...
- **Study day + rounds** (`recordings.db`): the study day is **anchored on the recording session, not on individual frame timestamps** (rewritten 2026-08-08). A frame belongs to the local date of its session's Start tap (`frames.session` is that epoch; env `DRM_TZ`, default Europe/Berlin), so a session that runs past local midnight stays **one** study day instead of splitting into the field day plus a sliver on the next calendar date. The study day is the **latest such session date** at the moment round 1 is first opened. `sessionDayKey` (`server/src/time.ts`) is the single definition; `resolveDayScope` in `server/src/db.ts` turns a day key into its session set, chunk-window set, and capture range, and every day-scoped query (`aggregateFrameDays`, `listFramesOnDay`, `listPhotoFramesOnDay`, `listChunksOnDay`, `countFramesOnDay`) filters through it. `dayBoundsMs` returns the day's **epoch extent** — the local calendar day, extended past midnight by `DRM_DAY_OVERRUN_HOURS` (default 4, the diary-study day boundary) and further if the day's sessions recorded beyond that — which is what activity spans are validated against and what `/api/reconstruction/state` and the round payload expose as `dayStartMs`/`dayEndMs`. The overrun applies to **every** day, including one whose camera stopped before midnight: round 1 reconstructs from memory, not from camera coverage, so "I went to bed at 00:30" must remain expressible. The web app never re-derives the extent from the day key. Covered by `server/scripts/test-study-day.ts`. Study Settings' lab-test mode writes only below `<participant>/Test/` and creates no `frames`, `chunks`, `recording_events`, session summary, reminder eligibility, or VLM input, so it can run on Day 0 without becoming the study day. The participant-facing response list (`kind=self` for round 1, `kind=assisted` for round 2) **pins its `day` on first open**, even while it has zero activity children, so a new morning-after frame or frame deletion cannot shift a seen round; round 2 derives the same pinned day from round 1. There is no separate `reconstructions` table. `participants.arm` may remain in upgraded databases only as an ignored legacy column; it does not select the workflow. A legacy lab check accidentally run through the main Start Session flow still counts as research data and must be removed/reset before the field day.
- **API:** `GET /api/reconstruction/state` (day, counts, both rounds' status), `GET/PUT /api/reconstruction/round/:round` + `POST .../submit`. The API no longer exposes `mode`. **Round 2 reads AND writes 403 until round 1 is submitted** (the fixed-order invariant, server-side). Round 1 carries no `frames`, no `vlmPendingCount`, rejects `source:'vlm'` rows, and strips VLM-provenance echoes; round 2 serves frames + VLM labels + the segmentation bootstrap. `GET /api/study/status` exposes the authenticated participant's persisted terminal state and photo access; `POST /api/study/complete` is idempotent, requires completed onboarding plus a submitted round 2, and sets `users.study_completed_at`. Once set, all reconstruction state/read/write/submit endpoints return `403` with `code='study_completed'`; profile, photo-listing/serving, and photo-deletion APIs remain available.
- **Photo management and frame contract (added 2026-07-28, restored and regression-tested 2026-07-30; redesigned 2026-08-03):** photo access starts only after round 1 is submitted and remains the sole study-data interaction after final completion. The same authoritative gate protects the mobile session-frame listing, raw `/frames/*` serving, deletion routes, and web `GET /api/photos`; session summaries alone remain visible before submission. `GET /api/photos` returns every frame row on the pinned study day for the navbar's **Manage Photos** gallery, while **View all photos** and thumbnail clicks open the same shared gallery scoped to an activity. Round-2 and photo responses identify every frame with `device`, `session`, and `frameIndex`, plus `captureEpochMs`, nullable `imageUrl`, and `deletedAt`. Single and bounded multi-delete remove the JPEG, clear its serving path, and retain the database row as a timestamped tombstone; the UI also supports conventional anchor-based Shift selection across an inclusive range of live frames, skipping deleted placeholders. Tombstones preserve chronology, activity boundaries, and deleted-photo counts, but never expose a cleared path or image. The three image dialogs share the modern responsive shell and virtualized grid; selected start/end and every live frame between them use a thick inset blue selection ring, with blue Start/End chips, so edge selections are never clipped.
- **Merging adjacent assisted activities (added 2026-08-09):** the 5-minute segmentation splits a continuous stretch whenever the VLM's argmax wobbles, so every gap between two assisted activity cards carries a **Merge** control beside **Insert activity**. Merging extends the first row to the second row's end and drops the second; a gap left by an earlier deletion is absorbed so the span stays contiguous. The merged row keeps the first row's identity, `source`, and `proposal_activity_id` — the same rule split rows already follow — while participant answers survive only where the two rows agreed: activity and category are otherwise cleared, and a notice names what needs re-answering. The **experience rating is subordinate to the category** — only the rating matching the surviving category can survive (and only if both rows gave it the same answer), so a cleared category clears both ratings and the rating belonging to the other category is dropped rather than carried along. Analysis reads only the rating matching the final category, and the server stores both rating fields as sent, so this prevents two rows holding stale off-category answers from merging into a rating the participant never gave. The immutable `vlm_proposal` list is never touched, so label-quality analysis is unaffected. When an edit leaves two neighbours with the **same activity and the same category**, the Merge control highlights in amber with a "Same activity" chip (the shape of the red unassigned-images treatment, in a colour that reads as a suggestion and stays clear of the yellow dev-mode injected-row tint). Merging is never automatic: the participant may still be part-way through relabelling the pair. `mergeWithNextActivity` + `activitiesInviteMerge` in `drm-web/src/components/reconstruct/editor-types.ts`, covered by `drm-web/scripts/test-activity-merge.ts`.
- **DRM web presentation and interaction contract (redesigned 2026-08-02/03):** login, onboarding, instruction, editor, survey, and done routes share the BLINKS-logo navbar and quiet animated gradient background, with `prefers-reduced-motion` respected. The reconstruction progress bar spans the viewport directly below the navbar; the Self/Assisted editors do not add a redundant outer card. Assisted activity cards use a shallow edge-to-edge photo strip with floating time/actions, followed by compact Activity, Activity Type, and rating rows whose controls align to the trailing edge and whose dropdowns share a width. Activity deletion requires a confirmation dialog; destructive icon hover states are immediate solid fills rather than translucent animations. The all-day and activity photo dialogs plus Adjust Times use the same modal language and virtualized image rendering so scrolling does not repeatedly decode or remount every frame. Login and done avoid nested cards, and the final-survey page centers its external-link CTA while trailing-aligning the completion CTA with a right-arrow icon.
...
- **Research admin (`/admin`):** a persisted `users.role = participant | admin` keeps admin and participant bearer tokens mutually exclusive. Admins have a separate login/token cookie and can page through the raw `participants`, `frames`, `chunks`, `activity_lists`, and `activities` tables, download each complete table as CSV, and inspect only live face-anonymized images with participant/session filters. The panel also provisions participant auth + recordings-profile rows with a temporary password and mandatory first-login onboarding; duplicate IDs are never overwritten. Admin profiles live only in `auth.db` and are created/reset with `npm run create-admin -- <username> <password> [--reset]`.
...
- **App:** onboarding blocks the tabs only after the profile has _loaded_ with occupation OR wake/bed times missing (offline never locks the recorder); wake/bed are HH:MM text inputs validated client- and server-side. Dashboard shows the single recording day (no N-day circles; `studyDurationDays` removed from `/api/profile` and `appConfig`). Profile links to a PIN-protected **Study Settings** subpage containing the participant-level camera form factor (`necklace | glasses`, persisted in `participants.camera_form_factor` and editable at any time), the isolated Day-0 test recording, and clockwise image rotation (`None | 90° | 180° | 270°`); the recorder reads the current rotation for every received frame, so changes during an active recording affect the next camera image before WebSocket upload. `EXPO_PUBLIC_STUDY_SETTINGS_PIN` configures the page PIN and defaults to `2626`; because it is bundled into the app, it is an access deterrent rather than a server secret. **expo-notifications, expo-file-system, and expo-image-manipulator are native modules → the phones need a fresh dev build** (`npx expo run:android`).
...
**DRM web developer navigator (2026-07-19, onboarding/intro previews added 2026-08-02):** start both the API server and `drm-web` with `DRM_DEV_MODE=1` (or run `DRM_DEV_MODE=1 ./dev-all.sh`) to show a floating menu linking directly to the non-mutating onboarding preview, Self-DRM introduction, Self editor, assisted introduction, Assisted editor, survey, and done pages. The onboarding/intro previews are fully interactive but never change the signed-in account; editor links still use real signed-in data. Dev mode bypasses only the evening gate and round-1-before-round-2 gate—authentication, participant isolation, autosave/submission, terminal study completion, and submitted-round finality remain enforced. Never enable it during the study.
...
- `server/vlm/` — VLM scene-understanding worker (Python; KIT SCC AI toolbox, OpenAI-compatible, Qwen3.5 `kit.qwen3.5-397b-A17b`). Labels completed 5-minute chunks with multi-image requests. Separate process; runs in parallel with face-blur but waits until attached frames leave pending/processing face status.
...
- Originally planned as a **5-day study**; the current DRM subproject runs **one field day and one recording session per participant** (see CURRENT PRIORITY). ~20 participants self-administer that session through the app. The multi-day shape returns with the full adaptive-ESM version.
...
6. **Camera software standby (implemented 2026-08-05; hardware battery/image validation pending):** the firmware detects OV2640 vs. OV3660 and uses the sensor-specific SCCB standby bit because the XIAO exposes no hardware PWDN GPIO. The sensor sleeps while disconnected, paused, and between samples; it wakes `CAMERA_WARMUP_MS=500` before a due capture so AEC/AWB can adjust. After copying the fresh JPEG, the driver parks one framebuffer and the sensor returns to standby before BLE sends the copy. A failed standby write disables duty cycling without stopping capture; a failed wake reinitializes the camera driver.
7. **BLE connected-idle sleep (implemented 2026-08-05; hardware current/link validation pending):** keep the phone connection instead of advertising/reconnecting every 15 seconds. Sketch-local `build_opt.h` pre-includes `ble_power_config.h`, which initializes the ESP32-S3 Bluetooth controller with modem sleep using the main crystal. The peripheral requests a 30-50 ms connection interval, slave latency 9, and 6 s supervision timeout; the phone owns the final negotiation. Startup and parameter callbacks log whether modem sleep and the requested parameters were accepted. Maximum transmit power remains unchanged until body-worn range is measured.
...
- **Routes:** `login`, `(tabs)/` = Dashboard / Photos / Profile, `recording` (full-screen modal), `session-detail`. The internal route filename remains `history.tsx`, but all participant-facing navigation says Photos. Auth-gated via `Stack.Protected` in `src/app/_layout.tsx`; token in SecureStore; any 401 auto-signs-out.
...
- **Dashboard:** single recording-day card (not started / restoring / recording / completed), one-session-per-participant gate, Start/Return-to-session button. An unfinished session remains reopenable after the app process is closed; only the explicit End action makes the session final. (The multi-day progress circles were removed in the 2026-07-12 single-day rewrite.)
- **Study Settings lab test:** before the main session exists, Profile → Study Settings exposes **Start test recording**. It reuses the production BLE, foreground-service, uploader, pause/resume, counters, recording screen, and image-rotation path, and is visibly marked `TEST RECORDING`. Test End is available at any time and resets only local test runtime; it never creates or completes the one allowed study session. Test sessions are intentionally not restored after app-process termination and may be repeated before the main session starts.
- **Recording screen:** full-screen, background animates green (`#15803D`) ↔ neutral (`#52525B`) on pause/resume, elapsed **active** (non-paused) time, camera/server/frame/queue status, Pause/Resume + End session. For the main recording, End session is disabled until 19:00 on the phone's local calendar day on which that session started; timestamp comparison keeps it available after midnight and on later recovery days. Afterward a warning-icon confirmation explains that ending is permanent and cannot be resumed. Test recordings bypass the 19:00 gate and use a separate confirmation that explicitly says the main session is unaffected. Main start/pause/resume/end are persisted as append-only `recording_events` with client and server timestamps plus a session-local sequence number; test actions are not. The app stores a main event in a per-participant SecureStore queue before delivery and retries it idempotently, so a temporary network failure does not erase the pause history. A compact per-participant active-main-session snapshot is refreshed every 30 seconds; after authentication the app reconciles it with `GET /api/recording/state`, recreates BLE + foreground-service + uploader state, and reuses the original session ID. Pause still writes the BLE control characteristic and drops in-flight frames. Main End records the terminal event even when currently paused, clears the server-side pause gate, and closes that session's last filling chunk.
- **Photos:** the session list is always visible, but `session-detail` first reads `GET /api/study/status` and shows an explicit recall-protection empty state while `canManagePhotos=false`. Once round 1 is submitted, the same screen enables a virtualized three-column `FlatList` gallery using the existing `expo-image` cache and full-screen zoom viewer; no additional gallery dependency is required. **Choose Multiple** enables whole-tile checkbox selection and bounded batch deletion; single deletion stays in the full-screen viewer. Deletion removes the JPEG, clears its serving path, and retains the frame row with `deleted_at` for audit/counting. Soft-deleted images are excluded from normal live-image reads, exports, face/VLM worker and segmentation input, and direct serving; their rows and timestamps remain visible as tombstones in DRM web frame/photo responses so chronology and activity boundaries stay stable. The app receives no VLM output.
- **Capture core (`src/capture/`):** `recording-session-store.ts` is a module-level singleton wiring `CameraLink` (BLE central, reconnect loop) → `FrameAssembler` (tagged reassembly; **stamps capture time at header receipt** — within ~100 ms of true capture since the firmware sends the header right after capture; ESP32 has no clock) → `FrameUploader` (authenticated WS, in-memory queue ≤500 frames, 3 s reconnect, 20 s heartbeat). Notifee keeps that runtime alive while backgrounded; if Android terminates the process anyway, SecureStore + the server event stream restore the unfinished session on the next app launch. Frames from the terminated interval cannot be recovered, but subsequent frames append to the same server session and continue its frame numbering.
...
- **Auth:** separate **`server/data/auth.db`** (NOT in `recordings/` — backups/rsyncs of research data must never carry credentials). `users` (username PK = participant id, argon2id `password_hash`, `created_at`, `must_change_password`, nullable `onboarding_completed_at`, nullable `study_completed_at`) + `auth_tokens` (sha256-hashed opaque 32-byte tokens, no expiry for the short study, revocation = row delete). New accounts require the first-run web flow; migrated existing accounts are grandfathered as onboarding-complete but remain study-active until final confirmation. The token-authenticated onboarding password endpoint does not ask for the just-used lab password, while the general password endpoint still does. Login returns both onboarding and terminal-study state. Provisioning: `npm run create-user -- <username> <password> [--reset]`; full wizard/completion reset: `npm run reset-onboarding -- <username>` — no self-signup.
- **Endpoints** (bearer token; each participant sees only their own data): `POST /api/login`, first-run `GET /api/onboarding`, `POST /api/onboarding/password`, `POST /api/onboarding/complete`, `GET /api/study/status`, terminal `POST /api/study/complete`, general `POST /api/change-password`, `GET /api/sessions` (grouped from `frames`, with live + soft-deleted counts), `GET /api/sessions/:device/:session/frames`, `GET /api/photos` (full pinned-day audit), single `DELETE /api/sessions/:device/:session/frames/:frameIndex`, batch `DELETE /api/sessions/:device/:session/frames` with `{frameIndexes}` (both idempotent soft-delete paths: JPEG removed, row retained), `GET /frames/<file_path>` (ownership-checked file serving, deliberately not `express.static`), `GET /api/export.csv?device=&session=`, `GET /api/recording/state`, and the event endpoints `POST /api/recording/started`, `/api/pause`, `/api/resume`, `/api/recording/ended`. Recording state reports the authoritative unfinished/completed session, phase, next sequence, and active elapsed time; a second session ID is rejected. All photo listing, serving, and deletion endpoints return `403 code='photo_access_locked'` until round 1 is submitted; session summaries remain available. Every event body contains `eventId`, `session`, `clientEpochMs`, and `sequenceNumber`; replays are idempotent and conflicting ID/sequence reuse returns 409. `GET /health` is open.
- **Ingestion:** `WS /ingest?session=<epochSeconds>&device=<cameraId>&recordingType=study|test` with `Authorization: Bearer` on the upgrade; omitted `recordingType` defaults to `study` for older app compatibility. The **phone declares the session id** (epoch of the Start tap) so BLE/WS reconnects resume the same session. Study numbering continues from `MAX(frame_index)`. Test numbering resumes from filenames below `recordings/<participant>/Test/<device>/<session>/images/`; those JPEGs are never inserted into SQLite, served by an API, face-processed, chunked, sent to the VLM, or counted as a recording day/session. Test ingestion is rejected once a main `recording_events` session exists. Per frame: JSON `{"t":<phoneCaptureEpochMs>,"n":<cameraFrameCounter|null>}` then binary JPEG (same two-message shape as the old firmware). The server pause gate applies only to the persisted main session; test pause is enforced by the camera control plus the app's in-flight-frame drop. `device` = camera BLE MAC, colons stripped.
...
- **Model:** the **KIT SCC AI toolbox**, an **OpenAI-compatible** endpoint (`https://ki-toolbox.scc.kit.edu/api/v1`) — the same service + API key the sibling **KARMA** project uses (`openai` client, `client.chat.completions.create`). Default model **`kit.qwen3.5-397b-A17b`** (Qwen3.5 is multimodal and accepts the worker's full 20-image request). The endpoint is **KIT-hosted**, so even anonymized frames stay inside KIT infrastructure (not a public cloud) — this is what resolved the cloud-VLM data-protection question.
...
- **Lifecycle + retry audit (added 2026-08-04):** `chunks.status` is `ready → processing → done`; retryable failures return to `ready` with persisted 30 s, 2 min, 5 min, and 10 min delays, reaching terminal `failed` only after `VLM_MAX_ATTEMPTS=5`. `chunks` retains lifetime/current-cycle counters, next-attempt time, and last error type. Every call is retained in `vlm_attempts` with model, timing/duration, images sent, configured timeout, outcome, error class, and HTTP status; no raw response/error body is stored. Startup closes open attempt rows as `interrupted` and reclaims their chunks under the single-worker assumption.
...
- **XIAO ESP32S3 Sense** with an OV2640 or newer OV3660 camera on the Sense extension board; PSRAM 8 MB (OPI), needed for the camera framebuffer. External IPEX antenna required for radio stability.
...
- `camera-firmware/` needs the **NimBLE-Arduino** library (2.x API). Keep its `build_opt.h` and `ble_power_config.h` beside the sketch; ESP32 Arduino 3.3.8 uses them to enable BLE controller modem sleep without disconnecting the phone between frames.
...
- Camera config: VGA (640x480), JPEG quality 12, `CAMERA_GRAB_WHEN_EMPTY`, 1 PSRAM framebuffer, sensor-specific software standby with 500 ms warm-up.

---

## Verified status

- **2026-08-05: isolated lab-test recording verified.** The app reuses the BLE relay, foreground service, counters, and pause/resume/end screen in an explicit test mode, while the server writes reconnect-safe JPEGs only below `<participant>/Test/`. App TypeScript + Expo lint and server TypeScript build pass. The disposable full API/WebSocket/DRM smoke test proves three test frames survive a reconnect, create zero research `frames` rows and zero session summaries, leave `/api/recording/state` empty, and are rejected after the main session starts. Its control now lives in Profile → Study Settings beside the shared image-rotation setting.
...
- **Recording state is derived from `recording_events`.** At startup the server restores the current pause gate from each participant's latest event; `paused.json` is no longer read or written. The same append-only rows drive app-process restoration and support pause counts, intervals, incomplete-pause detection, client/server clock comparison, and session attribution.
...
npm run reset-onboarding -- participant1      # repeat onboarding and reopen a completed account
npm run test-auth-onboarding                  # auth migration + state tests
npx tsx scripts/test-study-day.ts             # session-anchored study day + past-midnight extent
...
# model defaults to kit.qwen3.5-397b-A17b via the KIT SCC AI toolbox — see vlm/README.md
```
